...
*/
nhn.husky.HuskyCore.addLoadedFile("hp_SE2M_Hyperlink2$Lazy.js");
/**
 ...
 */
nhn.husky.HuskyCore.mixin(nhn.husky.SE2M_Hyperlink2, {
	//@lazyload_js TOGGLE_HYPERLINK_LAYER,APPLY_HYPERLINK[
	$ON_TOGGLE_HYPERLINK2_LAYER : function(){
		if(!this.bLayerShown){
		...
		this.oApp.delayedExec("TOGGLE_TOOLBAR_ACTIVE_LAYER", [this.oHyperlinkLayer, null, "MSG_HYPERLINK2_LAYER_SHOWN", [], "MSG_HYPERLINK2_LAYER_HIDDEN", [""]], 0);
		this.oApp.exec('MSG_NOTIFY_CLICKCR', ['hyperlink2']);
	},
	
	$ON_MSG_HYPERLINK2_LAYER_SHOWN : function(){
		...
		this.oApp.delayedExec("SELECT_UI", ["hyperlink2"], 0);
	...
	$ON_MSG_HYPERLINK2_LAYER_HIDDEN : function(){
		this.bLayerShown = false;
		
		this.oApp.exec("DESELECT_UI", ["hyperlink2"]);
	},
	...
	$ON_APPLY_HYPERLINK2 : function(){
		
		// [SMARTEDITORSUS-1451] 글감에 링크를 적용하지 않도록 처리
		if(!this._validateTarget()){
			alert(this.oApp.$MSG("SE_Hyperlink2.invalidTarget"));
			return;
		}
		...
		if(this._validateURL(sURL)){
			...
			this.oApp.exec("RECORD_UNDO_BEFORE_ACTION", ["HYPERLINK2", {sSaveTarget:(this.bModify ? "A" : null)}]);
			...
			setTimeout(jindo.$Fn(function(){
				...
				this.oApp.exec("RECORD_UNDO_AFTER_ACTION", ["HYPERLINK2", {sSaveTarget:(this.bModify ? "A" : null)}]);
			}, this).bind(), 17);			
		}else{
			alert(this.oApp.$MSG("SE_Hyperlink2.invalidURL"));
			this.oLinkInput.focus();
		}
	},
